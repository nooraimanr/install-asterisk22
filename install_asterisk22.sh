...
if ! $SKIP_CONFIRM && ! $DRY_RUN; then
  ...
  echo
  # When piped via curl | bash, stdin is the pipe — read from /dev/tty instead
  if [[ -t 0 ]]; then
    read -rp "Continue? [y/N] " _CONFIRM
  elif [[ -e /dev/tty ]]; then
    read -rp "Continue? [y/N] " _CONFIRM < /dev/tty
  else
    # No interactive terminal available (e.g. CI); require --skip-confirm
    die "No interactive terminal detected. Re-run with --skip-confirm to proceed non-interactively."
  fi
